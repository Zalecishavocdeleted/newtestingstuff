  ...
  create() {
    ...
    this._openSearchMenu = () => {
      ...
      const _doSearchInner = async (levelId) => {
       ...
       // ── jsDelivr level cache ──────────────────────────────────────────────
const JSDELIVR_URL = `https://cdn.jsdelivr.net/gh/alecishavoc/gddatabase-webdashers@main/level/${levelId}.json`;
let rawResponse = null;
try {
  _showStatus("checking jsdelivr cache...", "#ffb700");
  const jsdRes = await fetch(JSDELIVR_URL);
  if (jsdRes.ok) {
    const jsdText = await jsdRes.text();
    if (jsdText && jsdText.includes(":")) {
      rawResponse = jsdText;
      console.log("loaded level from jsdelivr:", JSDELIVR_URL);
    }
  }
} catch (jsdErr) {
  console.warn("jsdelivr fetch failed, falling back to proxy:", jsdErr);
}
// ── proxy fallback (if jsdelivr didn't work) ──────────────────────────
if (!rawResponse) {
          ...
          try {
            ...
            if (ngText && ngText !== "-1") { 
              ...
              if (songUrl) {
                ...
                // ── jsDelivr song cache ───────────────────────────────────────────────
const JSDELIVR_SONG_URL = `https://cdn.jsdelivr.net/gh/alecishavoc/gddatabasesongs-webdashers@main/songs/${songIdRaw}.mp3`;
let arrayBuf = null;
try {
  _showStatus(`checking jsdelivr for song #${songIdRaw}...`, "#ffb700");
  const jsdSongRes = await fetch(JSDELIVR_SONG_URL);
  if (jsdSongRes.ok) {
    arrayBuf = await jsdSongRes.arrayBuffer();
    console.log("loaded song from jsdelivr:", JSDELIVR_SONG_URL);
    _showStatus(`got "${songTitle}" from cache!`, "#00ff00");
  } else throw new Error("not in jsdelivr");
} catch (jsdSongErr) {
  console.warn("song not in jsdelivr, fetching from proxy:", jsdSongErr);
}
// ── proxy fallback ────────────────────────────────────────────────────
if (!arrayBuf) {
