...
// CBF — raw keydown listener queues presses between physics frames
window.cbf = true;
this._cbfPressQueued = false;

if (!window._gdCBFListenerAdded) {
  window._gdCBFListenerAdded = true;
  const _cbfCodes = new Set(['Space', 'ArrowUp', 'KeyW', 'KeyL']);
  document.addEventListener('keydown', (e) => {
    if (!window.cbf || e.repeat) return;
    if (_cbfCodes.has(e.code) && window._gdCBFScene) {
      window._gdCBFScene._cbfPressQueued = true;
    }
  });
}
window._gdCBFScene = this;
...
_buildSettingsPopup() {
 ...
 _buildHowToPlayPopup() {
  ...
  _resetGameplayState() {
    ...
    this._spaceWasDown = false;
    this._cbfPressQueued = false; // CBF
  ...
  const effectiveDown = window.cbf ? (jumpHeld || this._cbfPressQueued) : jumpHeld;

  if (!this._updateLogPopup && effectiveDown && !this._spaceWasDown) {
    this._pushButton();
  } else if (!effectiveDown && this._spaceWasDown) {
    this._releaseButton();
  }

  this._cbfPressQueued = false;
  this._spaceWasDown = effectiveDown;
};
